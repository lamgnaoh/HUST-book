...
const connect = (container, mediator) => {
  ...
  const opts = { ...dbSettings, db: 'aedes' }
  const connectionStringAedes = getMongoURL(opts)
  container.registerValue('connectionString', connectionString)
  container.registerValue('connectionStringAedes', connectionStringAedes + '?authSource=admin')
